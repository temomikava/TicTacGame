...
namespace WebAPI.SignalR
{
    ...
    public class GameHub : Hub
    {
        ...
        public async Task JoinToGame(int gameId)
        {
            ...
            await Clients.User(mainGame.PlayerTwo.Id.ToString()).SendAsync("nextturn", mainGame.PlayerTwo.UserName + "`s turn",1);
        ...
        public async Task MakeMove(int gameId, int r, int c)
        {
            ...
            if (mainMatch == null)
            {
                await Clients.Caller.SendAsync("nextturn", "match is not started",-1);
                return;
            }
            ...
            {
                await Clients.Caller.SendAsync("nextturn", "wait for your turn",-1);
            ...
            else
            {
                ...
                {
                   await Clients.Caller.SendAsync("nextturn", makeMove.ErrorMessage,-1);
                   return;
                }
                if (mainMatch.MatchOver!=true)
                {
                    ...
                    await Clients.Caller.SendAsync("nextturn", opponent.UserName + "`s turn",1);
                    await Clients.User(opponent.Id.ToString()).SendAsync("nextturn", "your turn", 1);
